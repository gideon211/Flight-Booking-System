...
const Navbar = () => {
  ...
  const handleFlightsClick = () => {
    navigate("/Availableflights", { state: { results: [] } }); // empty results for now
  };
    ...
    <nav className="flex justify-between items-center px-6 md:px-8 h-[4.5rem] bg-gradient-to-r from-blue-400 to-indigo-500 text-white shadow-lg">
      ...
      <div className="flex items-center gap-4">
        ...
        <select
          ...
          className="hidden sm:block bg-transparent border border-white/40 px-2 py-1 rounded text-sm cursor-pointer text-gray-300 outline-none"
        >
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            <span className="hidden sm:inline font-medium">{user ? user.name : "Account"}</span>
          </button>

          {dropdownOpen && (
            <div className="absolute right-1 mt-2 w-64 bg-white text-gray-800 rounded shadow-xl overflow-hidden z-50 cursor-pointer">
              {!user ? (
                <div className="flex flex-col font-medium">
                  <Link to="/Login" className="px-4 py-3 hover:bg-gray-200 transition">Login</Link>
                  <Link to="/SignUp" className="px-4 py-3 hover:bg-gray-200 transition">Register</Link>
                  <Link to="/TicketSection" className="px-4 py-3 hover:bg-gray-200 transition">My Tickets</Link>
                </div>
              ) : (
                <div>
      ...
      {/* Mobile Dropdown */}
      {mobileMenuOpen && (
        <div className="absolute top-[4.5rem] left-[13rem] w-48 bg-white text-gray-800 shadow-md md:hidden z-40 rounded">
          <ul className="flex flex-col font-medium">
            <li onClick={handleFlightsClick} className="px-6 py-3 border-b hover:bg-gray-100 cursor-pointer">
              Flights
            </li>
            <li className="px-6 py-3  hover:bg-gray-100 cursor-pointer">{t("Hotels")}</li>
            <li className="px-6 py-3  hover:bg-gray-100 cursor-pointer">{t("Packages")}</li>
            <li className="px-6 py-3  hover:bg-gray-100 cursor-pointer">{t("Car Rentals")}</li>
          </ul>
        </div>
      )}
    </nav>
